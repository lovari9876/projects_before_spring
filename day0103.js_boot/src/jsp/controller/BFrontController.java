...
//		viwPage에다가 웹컨텐트 이하!!!! 부를 파일폴더 해서 파일명 넣어주기!!!!!!!
		if (com.equals("/write_view.do")) {
			viewPage = "/jsp.mvc_board/write_view.jsp";
		} else if (com.equals("/write.do")) {
			command = new BWriteCommand();
			command.execute(request, response);

			viewPage = "list.do"; // 쓰고 나면 글 목록 보여줘
		} else if (com.equals("/list.do")) {
			command = new BListCommand();
			command.execute(request, response);

			viewPage = "/jsp.mvc_board/list.jsp";
		} else if (com.equals("/content_view.do")) {
			command = new BContentCommand();
			command.execute(request, response);

			viewPage = "/jsp.mvc_board/content_view.jsp";
		} else if (com.equals("/modify.do")) {
			command = new BModifyCommand();
			command.execute(request, response);

			// 수정 페이지가 따로 있지 않아..
			// 그냥 content-view에서 수정하고 수정버튼 submit 하면
			// modify.do로 가서 실행후, list.do로 응답...
			viewPage = "list.do";
			...
			viewPage = "/jsp.mvc_board/reply_view.jsp";
